...
MAX_TIMES = 1  # 2 times for dwt 10 for dct (0 represent 1, 1 represent 2)
...
def add_dwt_wm(img_ori_PATH, img_wm_PATH,
...
def use_dwt(img_ori_PATH, img_wm_PATH,
            strength_R, strength_G, strength_B,
            ori_CLASS, wm_CLASS, TIMES, img_output_PATH, eng, class_name):
    img_transfer = add_dwt_wm(img_ori_PATH, img_wm_PATH,
                              strength_R, strength_G, strength_B,
                              ori_CLASS, wm_CLASS, TIMES, img_output_PATH, eng)
    ...
    if class_wmed == ori_CLASS:
        for i in range(0, MAX_TIMES):
            if i <= MAX_TIMES:
                img_transfer = add_dwt_wm(img_transfer, img_wm_PATH,
                                          strength_R, strength_G, strength_B,
        ...
        for i_ori in files_ori:
            ...
            for folder_wm_img in folder_wm_list:  
                ...
                for i_wm in files_wm:  
                    ...
                    time_check = 1
                    round_result = use_dwt(current_ori_path, current_wm_path, R, G, B, class_ori, class_wm, time_check, buff_img_path, matlab_eng, class_name)
